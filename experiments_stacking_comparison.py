...
def run_estimator(name: str, estimator, sampler, fresh_draws_dir=None) -> Dict[str, Any]:
        ...
        if name == "StackedDR" and result.metadata:
            if 'stacking_weights' in result.metadata:
                stacking_info['weights'] = {
                    policy: {est: float(w) for est, w in zip(result.metadata.get('valid_estimators', []), weights)}
            ...
            if 'failed_estimators' in result.metadata:
                stacking_info['failed_estimators'] = result.metadata['failed_estimators']
...
def main():
    ...
    estimators = {
        ...
        "DR-CPO": DRCPOEstimator(sampler, calibrator=cal_result.calibrator if cal_result else None),
        "TMLE": TMLEEstimator(sampler, calibrator=cal_result.calibrator if cal_result else None),
        "MRDR": MRDREstimator(sampler, calibrator=cal_result.calibrator if cal_result else None),
    }
    ...
    for name, estimator in estimators.items():
        ...
        results[name] = run_estimator(
            ...
            estimator, 
            sampler,
            FRESH_DRAWS_DIR if needs_fresh else None
        )
    ...
    for name, result in results.items():
        if result['success']:
            for policy in sampler.target_policies:
                ...
                logger.info(f"{name:<15} {policy:<25} {est:.4f} [{ci_lower:.4f}, {ci_upper:.4f}]  {time_str:<10}")
        else:
            logger.info(f"{name:<15} {'FAILED':<25} {result.get('error', 'Unknown error')[:40]}")
    ...
    if 'StackedDR' in results and results['StackedDR']['success']:
        ...
        if 'failed_estimators' in stacking_info:
            logger.info(f"Failed estimators: {', '.join(stacking_info['failed_estimators'])}")
