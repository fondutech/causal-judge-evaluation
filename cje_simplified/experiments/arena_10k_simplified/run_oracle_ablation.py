...
def run_single_experiment(
    ...
    # Run analysis
    cmd = [
        "poetry", "run", "python", "run_cje_analysis.py",
        "--data", str(data_file),
        "--estimator", estimator,
        "--n-folds", str(n_folds),
...
def main():
    parser = argparse.ArgumentParser(
        description="Run oracle coverage ablation study"
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
        ...
        for frac in ABLATION_CONFIG["oracle_fractions"]:
            for seed in ABLATION_CONFIG["seeds"]:
                ...
                if not output_file.exists():
                    cmd = [
                        "poetry", "run", "python", "prepare_ablation_data.py",
                        "--oracle-fraction", str(frac),
                        "--seed", str(seed),
                        "--output", str(output_file),
                    ]
    ...
    for policy in ["base", "clone", "unhelpful"]:
        ...
        for oracle_frac in sorted(df["oracle_fraction"].unique()):
            for estimator in ABLATION_CONFIG["estimators"]:
                ...
                row = df[(df["oracle_fraction"] == oracle_frac) & 
                ...
                if not row.empty:
                    ...
                    print(f"{oracle_frac:6.1%}   | {estimator['display']:14} | "
