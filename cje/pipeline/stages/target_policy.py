...
class TargetPolicyStage:
    ...
    def _compute_logprobs(
        ...
        """Compute log probabilities for all contexts."""
        ...
        with self.console.status(
            ...
            try:
                ...
                if isinstance(logprobs_result, dict):
                    # Format: {"logprobs": [[...], [...]], "responses": [[...], [...]]}
                    all_logprobs = logprobs_result["logprobs"]
                    all_responses = logprobs_result.get(
                        "responses", [[None] * num_policies] * len(contexts)
                    )
                elif isinstance(logprobs_result, list):
                    # Format: list of log probabilities
                    all_logprobs = logprobs_result
                    all_responses = [[None] * num_policies] * len(contexts)
                else:
                    raise ValueError(
                        f"Unexpected logprobs format: {type(logprobs_result)}"
                    )
