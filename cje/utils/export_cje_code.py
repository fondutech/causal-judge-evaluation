...
def get_files_from_directory(
    base_path: Path, directory: str, extensions: List[str]
) -> List[Tuple[Path, str]]:
...
def export_to_markdown(output_file: str = "cje_core_code.md"):
    ...
    for directory, file_path, relative_path in all_files:
        ...
        output.append(
            f"- [{relative_path}](#{relative_path.replace('/', '').replace('.', '').replace('_', '').lower()})"
        )
    ...
    print(
        f"✅ Successfully exported {file_count} files ({total_lines:,} lines) to {output_file}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Export CJE core code for sharing with chatbots"
    )
    parser.add_argument(
        "--output",
        "-o",
    ...
    print(
        "   Just open the file and copy everything, or use: cat cje_core_code.md | pbcopy"
    )
