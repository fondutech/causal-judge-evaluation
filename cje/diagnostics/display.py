...
def format_cfbits_summary(cfbits: "CFBitsDiagnostics") -> str:
    ...
    This provides the user's suggested format:
    CF-bits: X.X bits (W=0.XX from W₀=1.0). Wid=0.XX, Wvar=0.XX.
    IFR(OUA)=XX%, A-ESSF(LCB)=XX%. Gate: STATE. → Recommendation

    Args:
        cfbits: CF-bits diagnostics object

    Returns:
        Formatted one-line summary string
    """
    if TYPE_CHECKING:
        from .models import CFBitsDiagnostics, GateState

    # Start with CF-bits and width
    parts = []

    if cfbits.bits_tot is not None:
        parts.append(f"CF-bits: {cfbits.bits_tot:.1f} bits")

    if cfbits.w_tot is not None:
        parts.append(f"(W={cfbits.w_tot:.2f} from W₀={cfbits.w0:.1f})")

    # Width decomposition
    if cfbits.wid is not None and cfbits.wvar is not None:
        parts.append(f"Wid={cfbits.wid:.2f}, Wvar={cfbits.wvar:.2f}")

    # Efficiency metrics
    if cfbits.ifr_oua is not None:
        parts.append(f"IFR(OUA)={cfbits.ifr_oua:.0%}")
    elif cfbits.ifr_main is not None:
        parts.append(f"IFR={cfbits.ifr_main:.0%}")

    if cfbits.aess_oua is not None:
        parts.append(f"aESS(OUA)={cfbits.aess_oua:.0f}")
    elif cfbits.aess_main is not None:
        parts.append(f"aESS={cfbits.aess_main:.0f}")

    # Structural floors
    if cfbits.aessf_sigmaS_lcb is not None:
        parts.append(f"A-ESSF(LCB)={cfbits.aessf_sigmaS_lcb:.0%}")
    elif cfbits.aessf_sigmaS is not None:
        parts.append(f"A-ESSF={cfbits.aessf_sigmaS:.0%}")

    # Gate state
    parts.append(f"Gate: {cfbits.gate_state.value.upper()}")
    ...
    elif cfbits.gate_state == GateState.CRITICAL:
        parts.append("→ Use with extreme caution")
    elif cfbits.gate_state == GateState.WARNING:
        # Specific recommendation based on dominant channel
        if cfbits.dominant_channel == "identification":
            if cfbits.p_mass_unlabeled and cfbits.p_mass_unlabeled > 0.1:
                parts.append(
                    f"→ Add labels on unlabeled S-bins (p' mass {cfbits.p_mass_unlabeled:.0%})"
                )
            elif cfbits.labels_for_wid_reduction:
                parts.append(f"→ Add {cfbits.labels_for_wid_reduction} oracle labels")
            else:
                parts.append("→ Add more oracle labels")
        else:
            if cfbits.logs_factor_for_half_bit:
                parts.append(
                    f"→ {cfbits.logs_factor_for_half_bit:.1f}× logs for +0.5 bits"
                )
            else:
                parts.append("→ Collect more logs or use DR methods")
    else:
...
def format_cfbits_table(cfbits_list: list[Any]) -> str:
    ...
    """
    if not cfbits_list:
        return "No CF-bits diagnostics available"

    lines = []
    lines.append("=" * 100)
    lines.append("CF-BITS DIAGNOSTICS")
    lines.append("=" * 100)

    # Header
    lines.append(
        f"{'Policy':<20} {'Bits':<8} {'W_tot':<8} {'Wid':<8} {'Wvar':<8} "
        f"{'IFR(OUA)':<10} {'A-ESSF':<10} {'Gate':<10} {'Recommendation':<30}"
    )
    lines.append("-" * 100)

    for cf in cfbits_list:
        bits_str = f"{cf.bits_tot:.1f}" if cf.bits_tot is not None else "N/A"
        w_tot_str = f"{cf.w_tot:.3f}" if cf.w_tot is not None else "N/A"
        wid_str = f"{cf.wid:.3f}" if cf.wid is not None else "N/A"
        wvar_str = f"{cf.wvar:.3f}" if cf.wvar is not None else "N/A"

        ifr_str = "N/A"
        if cf.ifr_oua is not None:
            ifr_str = f"{cf.ifr_oua:.1%}"
        elif cf.ifr_main is not None:
            ifr_str = f"{cf.ifr_main:.1%}"

        aessf_str = "N/A"
        if cf.aessf_sigmaS_lcb is not None:
            aessf_str = f"{cf.aessf_sigmaS_lcb:.1%}"
        elif cf.aessf_sigmaS is not None:
            aessf_str = f"{cf.aessf_sigmaS:.1%}"

        gate_str = cf.gate_state.value.upper()

        # Short recommendation
        from .models import GateState

        if cf.gate_state == GateState.REFUSE:
            rec_str = "Do not use"
        elif cf.gate_state == GateState.CRITICAL:
            rec_str = "Use with caution"
        elif cf.gate_state == GateState.WARNING:
            if cf.dominant_channel == "identification":
                rec_str = (
                    f"Add labels ({cf.p_mass_unlabeled:.0%} unlabeled)"
                    if cf.p_mass_unlabeled
                    else "Add labels"
                )
            else:
                rec_str = (
                    f"{cf.logs_factor_for_half_bit:.0f}× logs"
                    if cf.logs_factor_for_half_bit
                    else "More logs"
                )
        else:
            rec_str = "Reliable"
        ...
        lines.append(
            f"{cf.policy:<20} {bits_str:<8} {w_tot_str:<8} {wid_str:<8} {wvar_str:<8} "
            f"{ifr_str:<10} {aessf_str:<10} {gate_str:<10} {rec_str:<30}"
        )
    ...
    # Add legend
    lines.append("\nLEGEND:")
    lines.append("  Bits: Total CF-bits of information (higher is better)")
    lines.append("  W_tot: Total width (Wid + Wvar)")
    lines.append("  Wid: Identification width (structural uncertainty)")
    lines.append("  Wvar: Sampling width (statistical uncertainty)")
    lines.append("  IFR(OUA): Information Fraction Ratio with Oracle Uncertainty")
    lines.append("  A-ESSF: Adjusted ESS Fraction on σ(S) (structural ceiling)")
    lines.append("  Gate: Reliability assessment (GOOD/WARNING/CRITICAL/REFUSE)")
