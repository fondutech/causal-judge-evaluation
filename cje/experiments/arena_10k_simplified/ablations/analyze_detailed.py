...
def print_rmse_tables(results: List[Dict[str, Any]]):
    ...
    for result in results:
        ...
        for policy in POLICIES:
            if policy in estimates and policy in oracle_truths:
                ...
                oracle_var_by_method_quad[method_key][quadrant][policy].append(
                    oracle_var
                )
        ...
        if all(p in estimates and p in oracle_truths for p in WELL_BEHAVED_POLICIES):
            ...
            rmse_by_method_quad[method_key][quadrant]["overall_sqerrs"].extend(
                overall_sqerrs
            )
            rmse_by_method_quad[method_key][quadrant]["overall_ovars"].extend(
                overall_ovars
            )
                ...
                for policy in POLICIES:
                    if quad_data[quad][policy]:
                        ...
                        if policy in oracle_var_by_method_quad[method][quad]:
                            policy_ovars[policy].extend(
                                oracle_var_by_method_quad[method][quad][policy]
                            )
    ...
    print(
        f"{'Method':<40} {'Overall':<8} {'Debiased':<8} {'Clone':<8} {'ParaU':<8} {'Premium':<8} "
...
def print_bias_tables(results: List[Dict[str, Any]]):
    ...
    print(
        f"{'':40} {'Bias (Std)':<12} {'Bias (Std)':<12} {'Bias (Std)':<12} {'Bias (Std)':<12}"
    )
    print("-" * 100)
    ...
    for _, row in df.iterrows():
        ...
        print(
            f"{row['method']:<40} {overall:<12} {clone:<12} {parau:<12} {premium:<12}"
        )
...
def print_coverage_tables(results: List[Dict[str, Any]]):
    ...
    for result in results:
        ...
        for policy in POLICIES:
            if all(
                policy in d for d in [estimates, oracle_truths, confidence_intervals]
            ):
...
def print_boundary_analysis_table(results: List[Dict[str, Any]]):
    ...
    print(
        f"{'Method':<40} {'Clone':<12} {'ParaU':<12} {'Premium':<12} {'Unhelpful':<12}"
    )
    print(
        f"{'':40} {'% Flagged':<12} {'% Flagged':<12} {'% Flagged':<12} {'% Flagged':<12}"
    )
    print("-" * 100)
    ...
    for method, stats in sorted(boundary_stats.items()):
        ...
        print(
            f"{row_vals[0]:<40} {row_vals[1]:<12} {row_vals[2]:<12} {row_vals[3]:<12} {row_vals[4]:<12}"
        )
...
def print_diagnostics_tables(results: List[Dict[str, Any]]):
    ...
    print(
        f"{'Method':<40} {'Clone':<10} {'ParaU':<10} {'Premium':<10} {'Unhelpful':<10}"
    )
    print("-" * 90)
    ...
    for method, diags in sorted(diag_by_method.items()):
        ...
        print(
            f"{method:<40} {ess_vals[0]:<10} {ess_vals[1]:<10} {ess_vals[2]:<10} {ess_vals[3]:<10}"
        )
    ...
    for method, diags in sorted(diag_by_method.items()):
        ...
        print(
            f"{method:<40} {tail_vals[0]:<10} {tail_vals[1]:<10} {tail_vals[2]:<10} {tail_vals[3]:<10}"
        )
...
def print_ranking_analysis(results: List[Dict[str, Any]]):
    ...
    for method, metrics in ranking_by_method.items():
        ...
        row = {
            ...
            "kendall_tau": (
                np.mean(metrics["kendall_tau"]) if metrics["kendall_tau"] else np.nan
            ),
            "spearman_rho": (
                np.mean(metrics["spearman_rho"]) if metrics["spearman_rho"] else np.nan
            ),
            "top1_accuracy": (
                100.0 * np.mean(metrics["top1_correct"])
    ...
    df = pd.DataFrame(summary_rows).sort_values(
        "kendall_tau", ascending=False, na_position="last"
    )
    ...
    print(
        f"{'Method':<40} {'Kendall τ':<10} {'Spearman ρ':<10} {'Top-1 Acc%':<10} {'N':<5}"
    )
    print("-" * 75)
