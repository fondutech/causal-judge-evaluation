...
class SampleSizeAblation(BaseAblation):
    ...
    def __init__(self):
        super().__init__(
            name="sample_size",
            cache_dir=Path("../.ablation_cache/sample_size")
        )
        ...
        for estimator in estimators:
            ...
            for n_samples in sample_sizes:
                ...
                if agg.get("n_seeds_successful", 0) > 0:
                    ...
                    mean_rmse = np.mean([r.get("rmse_vs_oracle", np.nan) for r in successful_results])
                    ...
                    logger.info(f"  Mean ESS: {mean_ess:.0f} ({100*mean_ess/n_samples:.1f}%)")
                else:
    ...
    def analyze_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for (estimator, n_samples), results_subset in sorted(by_config.items()):
            ...
            analysis[estimator]["mean_se"].append(np.nanmean(all_ses) if all_ses else np.nan)
            analysis[estimator]["mean_ess"].append(np.nanmean(all_ess) if all_ess else np.nan)
    ...
    def create_figure(self, results: List[Dict[str, Any]], output_path: Path = None):
        ...
        for estimator, data in analysis.items():
            if data["sample_sizes"]:
                ax.loglog(data["sample_sizes"], data["mean_rmse"], 
                          'o-', label=estimator.upper(), color=colors.get(estimator, 'gray'),
                          linewidth=2, markersize=8)
        ax.set_xlabel('Sample Size (n)', fontsize=12)
        ax.set_ylabel('RMSE vs Oracle', fontsize=12)
        ax.set_title('A. Estimation Error vs Sample Size', fontsize=14, fontweight='bold')
        ax.legend()
        ...
        for estimator, data in analysis.items():
            if data["sample_sizes"]:
                ax.loglog(data["sample_sizes"], data["mean_se"],
                          'o-', label=estimator.upper(), color=colors.get(estimator, 'gray'),
                          linewidth=2, markersize=8)
        ...
        for estimator, data in analysis.items():
            if data["sample_sizes"]:
                ax.plot(data["sample_sizes"], data["mean_ess"],
                       'o-', label=estimator.upper(), color=colors.get(estimator, 'gray'),
                       linewidth=2, markersize=8)
        ...
        for estimator, data in analysis.items():
            if data["sample_sizes"]:
                ax.semilogx(data["sample_sizes"], data["ess_percent"],
                           'o-', label=estimator.upper(), color=colors.get(estimator, 'gray'),
                           linewidth=2, markersize=8)
        
        ax.axhline(100, color='k', linestyle='--', alpha=0.5, label='Ideal (100%)')
        ax.axhline(10, color='r', linestyle='--', alpha=0.5, label='Gate threshold (10%)')
        ...
        plt.suptitle('Sample Size Requirements for CJE', fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
...
def main():
    ...
    # Compare efficiency
    if len(analysis) == 2 and all(len(data["mean_rmse"]) > 0 for data in analysis.values()):
        estimators = list(analysis.keys())
        
        # Find common sample sizes
        common_sizes = set(analysis[estimators[0]]["sample_sizes"]) & set(analysis[estimators[1]]["sample_sizes"])
