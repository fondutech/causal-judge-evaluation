...
def compute_diagnostic_metrics(
    ...
    if by_quadrant:
        ...
        for result in results:
            ...
            use_weight_cal = result.get("spec", {}).get("extra", {}).get("use_weight_calibration", False)
    ...
    for key, group_results in grouped_data.items():
        ...
        for policy in POLICIES:
            ...
            row[f"{policy}_tail_index"] = np.mean(hill_values) if hill_values else np.nan
            row[f"{policy}_hellinger"] = np.mean(hellinger_values) if hellinger_values else np.nan
            row[f"{policy}_max_weight"] = np.mean(max_weight_values) if max_weight_values else np.nan
...
def compute_boundary_analysis(
    results: List[Dict[str, Any]],
    calibrated_methods: Optional[List[str]] = None
) -> pd.DataFrame:
    ...
    for method_name in calibrated_methods:
        ...
        policy_dists: Dict[str, List[float]] = {
            policy: [] for policy in POLICIES
        }
        policy_flags: Dict[str, int] = {
        ...
        method_results = [
            r for r in results
            if r.get("spec", {}).get("estimator") == method_name
        ]
        ...
        for policy in POLICIES:
            if policy_dists[policy]:
                ...
                row[f"{policy}_pct_flagged"] = (
                    ...
                    if total_experiments > 0 else np.nan
                )
            else:
