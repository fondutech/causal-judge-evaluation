...
from .constants import POLICIES, WELL_BEHAVED_POLICIES, DEFAULT_ALPHA, Z_CRITICAL_95, DEFAULT_N_ORACLE
...
def compute_coverage_metrics(
    ...
    for result in results:
        ...
        ci_map = result.get("robust_confidence_intervals") or result.get("confidence_intervals") or {}
        ...
        for policy in POLICIES:
            if policy in estimates and policy in oracle_truths:
                ...
                if policy in ci_map and isinstance(ci_map[policy], (list, tuple)) and len(ci_map[policy]) == 2:
        ...
        rows.append({
            ...
            "use_weight_calibration": spec.get("extra", {}).get("use_weight_calibration", False),
    ...
    for result in results:
        ...
        for policy in POLICIES:
            if policy in estimates and policy in oracle_truths:
                ...
                interval_score = interval_width + undercoverage_penalty + overcoverage_penalty
...
def aggregate_coverage_by_estimator(df: pd.DataFrame, use_oracle_adjusted: bool = False) -> pd.DataFrame:
