...
def add_ablation_config(results: List[Dict[str, Any]]) -> None:
    ...
    for result in results:
        ...
        result["reward_calibration_mode"] = result.get("reward_calibration_used", "unknown")
        ...
        result["config_string"] = f"{spec.get('estimator')}_{weight_cal}_{iic}_{reward_mode}"
...
def filter_results(
    ...
    if estimator is not None:
        filtered = [r for r in filtered 
                   if r.get("spec", {}).get("estimator") == estimator]
    
    if sample_size is not None:
        filtered = [r for r in filtered 
                   if r.get("spec", {}).get("sample_size") == sample_size]
