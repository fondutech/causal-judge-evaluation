...
def compute_ranking_metrics(results: List[Dict[str, Any]]) -> Dict[str, pd.DataFrame]:
    ...
    return {"per_experiment": per_experiment_df, "aggregated": aggregated_df}


def compute_pairwise_preferences(results: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for result in results:
        ...
        for i, policy1 in enumerate(WELL_BEHAVED_POLICIES):
            for policy2 in WELL_BEHAVED_POLICIES[i + 1 :]:
                if (
                    policy1 in estimates
                    and policy2 in estimates
                    and policy1 in oracle_truths
                    and policy2 in oracle_truths
                ):
...
def compute_ranking_by_quadrant(results: List[Dict[str, Any]]) -> pd.DataFrame:
