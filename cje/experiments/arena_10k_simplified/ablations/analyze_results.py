...
def analyze_oracle_coverage(results: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    summary = df.groupby("Oracle Coverage").agg({
        "RMSE": ["mean", "std"],
        "CI Width": "mean",
...
def analyze_sample_size(results: List[Dict[str, Any]]) -> pd.DataFrame:
    ...
    for r in results:
        if r.get("success", False):
            ...
            rows.append({
                ...
                "RMSE": r.get("rmse_vs_oracle", np.nan),
                "Mean SE": np.mean(list(r.get("standard_errors", {}).values())) if r.get("standard_errors") else np.nan,
                "ESS": mean_ess,
                "ESS %": 100 * mean_ess / spec.get("sample_size", 1) if not np.isnan(mean_ess) else np.nan,
                "Runtime (s)": r.get("runtime_s", np.nan),
                "Seed": r.get("seed", 0)
            })
    ...
    summary = df.groupby(["Estimator", "Sample Size"]).agg({
        "RMSE": ["mean", "std"],
        "Mean SE": "mean",
        "ESS": "mean",
        "ESS %": "mean",
    ...
    for r in results:
        if r.get("success", False):
            ...
            rows.append({
                "Oracle Coverage": f"{spec['oracle_coverage']:.0%}",
                "Sample Size": spec.get("sample_size", r.get("n_samples", 0)),
                "N Oracle": int(spec['oracle_coverage'] * spec.get("sample_size", 0)),
                "RMSE": r.get("rmse_vs_oracle", np.nan),
                "CI Width": r.get("mean_ci_width", np.nan),
                "Runtime (s)": r.get("runtime_s", np.nan),
                "Seed": r.get("seed", 0)
...
def compute_key_findings(oracle_df: pd.DataFrame, sample_df: pd.DataFrame, interaction_df: pd.DataFrame):
        ...
        if rmse_col in oracle_df.columns:
            ...
            if len(rmse_values) > 1:
                ...
                if len(stable_idx) > 0:
                    ...
                    findings.append(f"Oracle coverage sweet spot: {sweet_spot} (diminishing returns beyond)")
                
                # Overall improvement from min to max coverage
                total_improvement = (rmse_values[0] - rmse_values[-1]) / rmse_values[0]
                findings.append(f"RMSE reduction from 1% to 100% oracle: {total_improvement:.1%}")
            ...
            if ("RMSE", "mean") in est_data.columns:
                ...
                if len(rmse_values) >= 2:
                    ...
                    if abs(slope + 0.5) < 0.1:  # Close to -0.5
                        findings.append(f"{estimator}: Follows √n scaling (slope={slope:.2f}, R²={r_value**2:.3f})")
                    else:
                        findings.append(f"{estimator}: Deviates from √n scaling (slope={slope:.2f})")
    ...
    if not interaction_df.empty:
        ...
        if best_configs:
            ...
            top = best_configs[0]
            findings.append(f"Most efficient config: {top[0]} oracle, n={top[1]} (RMSE={top[2]:.3f})")
        ...
        try:
            ...
            if oracle_results:
                ...
                ablation.create_figure(
                    oracle_results,
                    args.results_dir / "oracle_coverage" / "figure_1_oracle_coverage.png"
                )
