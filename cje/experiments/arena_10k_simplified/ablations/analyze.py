...
def main():
    ...
    if args.analysis in ["rmse", "all"]:
        ...
        if not rmse_df.empty:
            ...
            config_rmse = (
                rmse_df.groupby("config_string")["overall_rmse"]
                .agg(["mean", "std", "count"])
                .round(4)
            )
            ...
            estimator_rmse = (
                rmse_df.groupby("estimator")["overall_rmse"]
                .agg(["mean", "std", "count"])
                .round(4)
            )
    ...
    if args.analysis in ["diagnostics", "all"]:
        ...
        if args.verbose:
            ...
            if (
                ips_comparison.get("ess") is not None
                and not ips_comparison["ess"].empty
            ):
            ...
            for _, row in boundary_df.iterrows():
                ...
                for policy in ["clone", "parallel_universe_prompt", "premium"]:
                    ...
                    if dist_col in row and flag_col in row:
                        print(
                            f"  {policy}: dist={row[dist_col]:.3f}, flagged={row[flag_col]:.1f}%"
                        )
    ...
    if args.analysis in ["ranking", "all"]:
        ...
        if not ranking_results["aggregated"].empty:
            ...
            final_cols = [
                c for c in display_cols if c in ranking_results["aggregated"].columns
            ]
            print(
                ranking_results["aggregated"][final_cols]
                .round(3)
    ...
    if args.analysis == "all" and args.verbose:
        ...
        if not debiased_df.empty:
            ...
            print("=" * 80)
            debiased_summary = (
                debiased_df.groupby("estimator")["overall_rmse_debiased"]
                .mean()
                .sort_values()
            )
