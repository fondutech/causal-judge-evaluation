...
class InteractionAblation(BaseAblation):
    ...
    def __init__(self):
        super().__init__(
            name="interaction",
            cache_dir=Path("../.ablation_cache/interaction")
        )
    ...
    def run_ablation(self) -> List[Dict[str, Any]]:
        ...
        logger.info(f"Total configurations: {len(oracle_coverages) * len(sample_sizes)}")
        ...
        for oracle_coverage in oracle_coverages:
            for sample_size in sample_sizes:
                ...
                logger.info(f"\n{'='*60}")
                logger.info(f"Config {config_count}/{total_configs}: "
        ...
        for i, oracle in enumerate(oracle_values):
            for j, n_samples in enumerate(sample_values):
                if rmse_matrix[i, j] <= threshold:
                    ...
                    efficiency = 1.0 / (n_oracle * rmse_matrix[i, j])  # Higher is better
                    sweet_spots.append({
    ...
    def create_figure(self, results: List[Dict[str, Any]], output_path: Path = None):
        ...
        for i, oracle in enumerate(analysis["oracle_values"]):
            for j, n_samples in enumerate(analysis["sample_values"]):
                ...
                if np.isfinite(analysis["rmse_matrix"][i, j]):
                    efficiency_matrix[i, j] = 1000.0 / (n_oracle * analysis["rmse_matrix"][i, j])
                else:
                    efficiency_matrix[i, j] = np.nan
        
        # Create contour plot
        X, Y = np.meshgrid(range(len(sample_labels)), range(len(oracle_labels)))
        contour = ax.contour(X, Y, efficiency_matrix, levels=5, colors='black', alpha=0.4)
        ...
        ax.set_xlabel('Sample Size', fontsize=12)
        ax.set_ylabel('Oracle Coverage', fontsize=12)
        ax.set_title('B. Efficiency (Lower Cost × Error)', fontsize=14, fontweight='bold')
        
        plt.suptitle('Oracle × Sample Size Interaction', fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
...
def main():
    ...
    if len(analysis["rmse_matrix"]) > 0:
        ...
        if len(analysis["oracle_values"]) >= 2:
            ...
            if np.sum(valid) >= 2:
                improvement = (first_col[valid][0] - first_col[valid][-1]) / first_col[valid][0]
                logger.info(f"  Increasing oracle 10x: ~{improvement:.0%} RMSE reduction")
        ...
        if len(analysis["sample_values"]) >= 2:
            ...
            if np.sum(valid) >= 2:
                improvement = (first_row[valid][0] - first_row[valid][-1]) / first_row[valid][0]
                logger.info(f"  Increasing samples 25x: ~{improvement:.0%} RMSE reduction")
