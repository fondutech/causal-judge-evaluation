...
def print_detailed_rmse_analysis(results: List[Dict[str, Any]]):
    ...
    config_stats = rmse_df.groupby('config_string').agg({
    ...
    config_stats.columns = ['rmse_mean', 'rmse_std', 'rmse_min', 'rmse_max', 'n_exp',
                            'clone_rmse', 'para_rmse', 'premium_rmse', 'unhelpful_rmse']
    ...
    print("-" * 120)
    print(f"{'Configuration':<45} {'RMSE':<12} {'Clone':<10} {'ParaU':<10} {'Premium':<10} {'Unhelpful':<10} {'N':<5}")
    print("-" * 120)
    ...
    print(f"  • Best Overall RMSE: {best_overall} ({config_stats.loc[best_overall, 'rmse_mean']:.4f})")
    
    # Best for unhelpful policy
    best_unhelpful = config_stats.sort_values('unhelpful_rmse').index[0]
    print(f"  • Best for Unhelpful: {best_unhelpful} ({config_stats.loc[best_unhelpful, 'unhelpful_rmse']:.4f})")
    ...
    if wcal_configs and no_wcal_configs:
        ...
        if improvement > 0:
            print(f"  • Weight calibration improves RMSE by {improvement:.1f}% on average")
        else:
            print(f"  • Weight calibration hurts RMSE by {-improvement:.1f}% on average")
...
def print_coverage_analysis(results: List[Dict[str, Any]]):
    ...
    for result in results:
        idx = coverage_df[
            ...
            (coverage_df['seed'] == result.get('seed', result['spec'].get('seed_base', 0)))
        ].index
        if len(idx) > 0:
            coverage_df.loc[idx, 'config_string'] = result.get('config_string', 'unknown')
    
    # Compute coverage statistics by configuration
    print("\n📊 95% CI Coverage by Configuration (target = 95%):")
    print("-" * 120)
    print(f"{'Configuration':<45} {'Clone':<12} {'ParaU':<12} {'Premium':<12} {'Unhelpful':<12}")
    print("-" * 120)
    ...
    for config in coverage_df['config_string'].unique():
        ...
        print(f"{display_name:<45} {coverages[0]:<12} {coverages[1]:<12} {coverages[2]:<12} {coverages[3]:<12}")
    ...
    if high_variance_policies:
        print(f"  • High coverage variance across configs: {', '.join(high_variance_policies)}")
...
def print_diagnostics_summary(results: List[Dict[str, Any]]):
    ...
    for result in results:
        idx = diag_df[
            ...
            (diag_df['seed'] == result.get('seed', result['spec'].get('seed_base', 0)))
        ].index
        if len(idx) > 0:
    ...
    for config in diag_df['config_string'].unique():
        ...
        for policy in ['clone', 'unhelpful']:
            ...
            if ess_col in config_data.columns:
                ...
                tail_alpha = config_data[tail_col].mean() if tail_col in config_data.columns else np.nan
                
                print(f"  {policy:<20}: ESS={ess:.1f}% {status}  Tail α={tail_alpha:.2f}")
...
def print_recommendations(results: List[Dict[str, Any]]):
    ...
    config_performance = rmse_df.groupby('config_string')['overall_rmse'].mean().sort_values()
    ...
    wcal_configs = [c for c in config_performance.index if 'WCal' in c and 'NoWCal' not in c]
