...
class EstimatorComparison(BaseAblation):
    ...
    def __init__(self):
        ...
        self.estimator_configs = [
            ...
            EstimatorConfig("cal-dr-cpo", "Cal-DR-CPO", "dr-cpo", True, "hajek", is_dr=True),
            EstimatorConfig("stacked-dr", "Stacked-DR", "stacked", False, "hajek", is_dr=True, is_stacked=True),
            EstimatorConfig("cal-stacked-dr", "Cal-Stacked-DR", "stacked", True, "hajek", is_dr=True, is_stacked=True),
        ]
    
    def create_custom_estimator(self, config: EstimatorConfig, sampler: PrecomputedSampler, cal_result: Any):
    ...
    def run_single_comparison(self, spec: ExperimentSpec, config: EstimatorConfig, seed: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if spec.sample_size:
                ...
                indices = sorted(np.random.choice(len(dataset.samples), n, replace=False))
                dataset.samples = [dataset.samples[i] for i in indices]
            
            # Mask oracle labels for calibration
            n_samples = len(dataset.samples)
            if spec.oracle_coverage and spec.oracle_coverage < 1.0:
                oracle_indices = [i for i, s in enumerate(dataset.samples) 
    ...
    def run_ablation(self) -> List[Dict[str, Any]]:
        ...
        for scenario in scenarios:
            logger.info(f"\n{'='*60}")
            logger.info(f"Scenario: {scenario['label']} (n={scenario['n']}, oracle={scenario['oracle']:.0%})")
            logger.info(f"{'='*60}")
            ...
            for config in self.estimator_configs:
                ...
                for seed_offset in range(n_seeds):
                    ...
                    if result["success"]:
                        ...
                        logger.info(f"  Seed {seed}: {mean_est:.3f} ± {mean_se:.3f} ({result['runtime']:.1f}s)")
                    else:
        ...
        for r in results:
            if r.get("success", False):
                ...
                by_scenario[scenario][estimator].append({
                    "se": mean_se,
                    "runtime": r["runtime"],
                    "ess": np.mean(list(r.get("ess", {}).values())) if "ess" in r else None
                })
    ...
    def create_figure(self, results: List[Dict[str, Any]], output_path: Path = None):
        ...
        plt.suptitle("Estimator Comparison: Standard Error by Scenario", fontsize=16, fontweight='bold')
        plt.tight_layout()
...
def main():
    ...
    for scenario, ranking in rankings.items():
        ...
        for i, r in enumerate(ranking, 1):
            logger.info(f"{i}. {r['estimator']:15s}: SE={r['mean_se']:.4f}, Runtime={r['mean_runtime']:.1f}s")
    
    # Create figure
    figure_path = Path("../ablations/results/estimator_comparison/comparison_figure.png")
