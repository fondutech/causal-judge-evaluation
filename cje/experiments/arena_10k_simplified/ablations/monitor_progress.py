...
def main():
    ...
    while True:
        ...
        for ablation, count in counts.items():
            pct = (count["successful"] / expected[ablation] * 100) if expected[ablation] > 0 else 0
            bar_length = int(pct / 5)
            bar = "█" * bar_length + "░" * (20 - bar_length)
            print(f"{ablation:20s}: [{bar}] {count['successful']:3d}/{expected[ablation]:3d} ({pct:5.1f}%)")
        ...
        if total_successful > 0 and is_running:
            ...
            rate = total_successful / (runtime.total_seconds() / 60)  # experiments per minute
            remaining = total_expected - total_successful
            eta_minutes = remaining / rate if rate > 0 else 0
            eta = datetime.now() + timedelta(minutes=eta_minutes)
            print(f"Estimated completion: {eta.strftime('%H:%M:%S')} ({eta_minutes:.0f} minutes)")
